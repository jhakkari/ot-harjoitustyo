...
class SnippetListView:
    ...
    def _initialize_empty_snipped_block(self):
        ...
        content_label = ttk.Label(
            master=snippet_block_frame, text="Nothing in here, yet!")
    ...
    def _initialize_snippet_block(self, snippet):
        ...
        snippet_block_frame = ttk.Frame(master=self._frame)
        content_label = ttk.Label(
            master=snippet_block_frame, text=snippet.content)
        created_at_label = ttk.Label(
            master=snippet_block_frame, text=f"Added at: {snippet.created_at}")

        copy_to_clipboard_button = ttk.Button(
            master=snippet_block_frame, text="Copy", command=lambda: self._handle_copy_to_clipboard(snippet.content))
        remove_snippet_button = ttk.Button(
            master=snippet_block_frame, text="Delete", command=lambda: self._handle_remove_sippet(snippet.snippet_id))

        content_label.grid(row=0, column=0, padx=5, pady=5, sticky=constants.W)
        created_at_label.grid(row=1, column=0, padx=5,
...
class MainView:
    ...
    def _initialize_snippet_list(self):
        ...
        self._snippet_list_view = SnippetListView(self._snippet_list_frame, snippet_service.get_snippets_list(
        ), self._handle_copy_snippet_to_clipboard, self._handle_remove_snippet)
    ...
    def _initialize_action_feedback_area(self):
        ...
        self._action_feedback_label.grid(
            row=0, column=0, columnspan=2, sticky="ewns")

    def _initialize_action_buttons(self):
        """Alustaa näkymän vasemmanpuoleisessa palkissa olevat nappulat omaan frameen.
        """
        logout_button = ttk.Button(
            self._action_buttons_frame, text="Logout", command=self._handle_logout)
        add_snippet_button = ttk.Button(
            self._action_buttons_frame, text="Add snippet", command=self._handle_show_add_snippet_view)
        settings_button = ttk.Button(
            self._action_buttons_frame, text="Settings", command=self._handle_show_settings_view)
    ...
    def _initialize(self):
        ...
        self._action_feedback_frame.grid(
            row=1, column=0, columnspan=2, padx=5, pady=5, sticky="s")
