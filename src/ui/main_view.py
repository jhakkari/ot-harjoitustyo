...
class SnippetListView:
    ...
    def _initialize_empty_snipped_block(self):
        ...
        content = ttk.Label(master=snippet_block_frame, text="Nothing in here, yet!")
        content.grid(row=0, column=0, padx=5, pady=5, sticky=constants.W)
    ...
    def _initialize_snippet_block(self, snippet):
        snippet_block_frame = ttk.Frame(master=self._frame)
        content = ttk.Label(master=snippet_block_frame, text=snippet.content)
        created_at = ttk.Label(master=snippet_block_frame, text=snippet.created_at)
        ...
        content.grid(row=0, column=0, padx=5, pady=5, sticky=constants.W)
        created_at.grid(row=1, column=0, padx=5, pady=5, sticky=constants.W)
...
class MainView:
    ...
    def _initialize(self):
        ...
        self._frame.columnconfigure(1, minsize=200, weight=1)
        ...
        logout_button = ttk.Button(buttons, text="Logout", command=self._handle_logout)
        logout_button.grid(row=0, column=0, sticky="ew", padx=5, pady=5)

        add_snippet_button = ttk.Button(buttons, text="Add snippet", command=self._handle_show_add_snippet_view)
        add_snippet_button.grid(row=1, column=0,sticky="ew", padx=5, pady=5)

        buttons.grid(row=0, column=0, sticky="ns")
        self._snippet_list_frame.grid(row=0, column=1, sticky="nsew")
